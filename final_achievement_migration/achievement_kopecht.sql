...
VALUES ('goals', 'Goals', 'Achievements for scoring goals', '⚽', 1)
ON CONFLICT (key) DO NOTHING;

-- Speed Category (for time-based achievements)
INSERT INTO kopecht.achievement_categories (key, name, description, icon, sort_order)
VALUES ('speed', 'Speed', 'Achievements for fast plays', '⚡', 2)
ON CONFLICT (key) DO NOTHING;

-- Matches Category (for match count achievements)
INSERT INTO kopecht.achievement_categories (key, name, description, icon, sort_order)
VALUES ('matches', 'Matches', 'Achievements for playing matches', '🎮', 3)
ON CONFLICT (key) DO NOTHING;

-- Wins Category (for winning achievements)
INSERT INTO kopecht.achievement_categories (key, name, description, icon, sort_order)
VALUES ('wins', 'Victories', 'Achievements for winning matches', '🏆', 4)
ON CONFLICT (key) DO NOTHING;

-- Skill Category (for skill-based achievements like MMR)
INSERT INTO kopecht.achievement_categories (key, name, description, icon, sort_order)
VALUES ('skill', 'Skill', 'Achievements for skill milestones', '📈', 5)
ON CONFLICT (key) DO NOTHING;

-- Comeback Category (for comeback achievements)
INSERT INTO kopecht.achievement_categories (key, name, description, icon, sort_order)
VALUES ('comeback', 'Comebacks', 'Achievements for epic comebacks', '🔥', 6)
ON CONFLICT (key) DO NOTHING;

-- Playtime Category (for time played achievements)
INSERT INTO kopecht.achievement_categories (key, name, description, icon, sort_order)
VALUES ('playtime', 'Playtime', 'Achievements for time played', '⏱️', 7)
ON CONFLICT (key) DO NOTHING;

-- Teamwork Category (for 2on2 specific achievements)
INSERT INTO kopecht.achievement_categories (key, name, description, icon, sort_order)
VALUES ('teamwork', 'Teamwork', 'Achievements for team play', '🤝', 8)
ON CONFLICT (key) DO NOTHING;

-- Streaks Category (for streak achievements)
INSERT INTO kopecht.achievement_categories (key, name, description, icon, sort_order)
VALUES ('streaks', 'Streaks', 'Achievements for win/loss streaks', '🔗', 9)
ON CONFLICT (key) DO NOTHING;

-- Season Category (for season achievements)
INSERT INTO kopecht.achievement_categories (key, name, description, icon, sort_order)
VALUES ('season', 'Season', 'Seasonal achievements', '🏅', 10)
ON CONFLICT (key) DO NOTHING;

-- Meta Category (for meta achievements)
INSERT INTO kopecht.achievement_categories (key, name, description, icon, sort_order)
VALUES ('meta', 'Meta', 'Achievements about achievements', '🎯', 11)
ON CONFLICT (key) DO NOTHING;

-- Secret Category (for hidden/joke achievements)
INSERT INTO kopecht.achievement_categories (key, name, description, icon, sort_order)
VALUES ('secret', 'Secret', 'Secret achievements', '🔮', 99)
ON CONFLICT (key) DO NOTHING;
...
-- 2. Die universelle Trigger-Funktion für Achievements im Schema 'kopecht'
create or replace function kopecht.trigger_process_achievement()
returns trigger 
language plpgsql
security definer -- WICHTIG: Läuft mit Rechten des Erstellers (Admin)
...
declare
  ...
  -- Dieser Key umgeht RLS und erlaubt den Zugriff auf die geschützte Function.
  ...
  -- Der Name des Webhooks wird als Argument übergeben
...
('title_carry', 'Carry', 'Show everyone you carry the team', 'title', 'prefix', 'Carry', 'carry_2on2', '💪', 10),

-- Giant Slayer achievement unlocks title
('title_giant_slayer', 'Giant Slayer', 'Display your prowess against stronger opponents', 'title', 'suffix', 'the Giant Slayer', 'giant_slayer_1on1', '⚔️', 20),

-- Win streak achievements
('title_unstoppable', 'Unstoppable', 'For those on an incredible winning streak', 'title', 'prefix', 'Unstoppable', 'win_streak_10_1on1', '🔥', 30),

-- Perfect games
('title_perfectionist', 'Perfectionist', 'For achieving perfect victories', 'title', 'suffix', 'the Perfectionist', 'perfect_wins_10', '✨', 40),

-- Speed demon - blitzkrieg is the fast win achievement
('title_speedster', 'Speedster', 'For lightning-fast victories', 'title', 'prefix', 'Speedster', 'blitzkrieg_1on1', '⚡', 50),

-- Comeback king
('title_comeback_king', 'Comeback King', 'For those who never give up', 'title', 'suffix', 'the Comeback King', 'miracle_comeback', '👑', 60),

-- Veteran title
('title_veteran', 'Veteran', 'For experienced players', 'title', 'prefix', 'Veteran', 'matches_1on1_100', '🎖️', 70),

-- Champion
('title_champion', 'Champion', 'For season champions', 'title', 'suffix', 'the Champion', 'season_champion_1on1', '🏆', 80),

-- Secret achievements - own goal specialist
('title_own_goal_master', 'Own Goal Specialist', 'Embrace your unique playstyle', 'title', 'suffix', 'the Own Goal Specialist', 'own_goals_alltime_100', '🙃', 90),

-- MMR Below 700 - for the meme
('title_underdog', 'Underdog', 'Started from the bottom', 'title', 'prefix', 'Underdog', 'mmr_below_700_1on1', '🐢', 100),
