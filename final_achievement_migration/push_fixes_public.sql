...
-- Migration: Add mention notifications system (public schema)
...
CREATE TABLE IF NOT EXISTS mention_notifications (
    ...
    sender_player_id BIGINT NOT NULL REFERENCES player(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_mention_notifications_user_unread ON mention_notifications(user_id, is_read) WHERE is_read = FALSE;
CREATE INDEX IF NOT EXISTS idx_mention_notifications_created_at ON mention_notifications(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_mention_notifications_kicker_id ON mention_notifications(kicker_id);
...
ALTER TABLE mention_notifications REPLICA IDENTITY FULL;
...
CREATE OR REPLACE FUNCTION mark_mention_as_read(p_notification_id BIGINT)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE OR REPLACE FUNCTION mark_all_mentions_as_read()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE OR REPLACE FUNCTION trigger_create_comment_mention_notifications()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE OR REPLACE FUNCTION trigger_create_chat_mention_notifications()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
GRANT EXECUTE ON FUNCTION mark_mention_as_read(BIGINT) TO authenticated;
GRANT EXECUTE ON FUNCTION mark_all_mentions_as_read() TO authenticated;

-- 13. Migrate historical mentions from match_comments
INSERT INTO mention_notifications (user_id, type, source_id, match_id, kicker_id, sender_player_id, content_preview, is_read, created_at)
SELECT DISTINCT
    p_mentioned.user_id,
    'comment',
    mc.id,
...
JOIN player p_mentioned ON p_mentioned.id = mentions.mentioned_player_id
...
SELECT DISTINCT
    ...
    mc.created_at
FROM match_comments mc
JOIN player p ON p.kicker_id = mc.kicker_id AND p.id != mc.player_id AND p.user_id IS NOT NULL
...
SELECT DISTINCT
    ...
    cm.created_at
FROM chat_messages cm
JOIN player p ON p.kicker_id = cm.kicker_id AND p.id != cm.player_id AND p.user_id IS NOT NULL
