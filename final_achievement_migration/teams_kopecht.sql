...
CREATE TABLE IF NOT EXISTS kopecht.team_status (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    team_id BIGINT NOT NULL REFERENCES kopecht.teams(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS kopecht.team_bounty_history (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    -- Team who claimed the bounty
    claimer_team_id BIGINT NOT NULL REFERENCES kopecht.teams(id) ON DELETE CASCADE,
    -- Team whose streak was broken
    victim_team_id BIGINT NOT NULL REFERENCES kopecht.teams(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_team_status_team ON kopecht.team_status(team_id);
CREATE INDEX IF NOT EXISTS idx_team_status_updated ON kopecht.team_status(updated_at);
CREATE INDEX IF NOT EXISTS idx_team_bounty_history_claimer ON kopecht.team_bounty_history(claimer_team_id);
CREATE INDEX IF NOT EXISTS idx_team_bounty_history_victim ON kopecht.team_bounty_history(victim_team_id);
CREATE INDEX IF NOT EXISTS idx_team_bounty_history_created ON kopecht.team_bounty_history(created_at);

-- 32. RLS Policies for team_status and team_bounty_history
ALTER TABLE kopecht.team_status ENABLE ROW LEVEL SECURITY;
ALTER TABLE kopecht.team_bounty_history ENABLE ROW LEVEL SECURITY;

-- Team status: Users can view team status from their kicker
CREATE POLICY "team_status_select_policy" ON kopecht.team_status
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM kopecht.teams t
            JOIN kopecht.player p ON p.kicker_id = t.kicker_id
...
CREATE POLICY "team_bounty_history_select_policy" ON kopecht.team_bounty_history
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM kopecht.teams t
            JOIN kopecht.player p ON p.kicker_id = t.kicker_id
...
DROP FUNCTION IF EXISTS kopecht.update_team_status_after_match(BIGINT, BIGINT, BOOLEAN, INT);

CREATE OR REPLACE FUNCTION kopecht.update_team_status_after_match(
    p_team_id BIGINT,
    p_match_id BIGINT,
    p_is_winner BOOLEAN,
...
BEGIN
    ...
    INSERT INTO kopecht.team_status (team_id, current_streak, current_bounty, active_statuses)
    ...
    IF p_is_winner AND p_opponent_team_id IS NOT NULL THEN
        ...
        IF v_opponent_status IS NOT NULL AND v_opponent_status.current_bounty > 0 THEN
            ...
            INSERT INTO kopecht.team_bounty_history (claimer_team_id, victim_team_id, match_id, streak_broken, bounty_amount)
...
CREATE OR REPLACE FUNCTION kopecht.get_team_bounty_for_team(
    p_team_id BIGINT
)
RETURNS INT AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION kopecht.get_player_team_stats(
    p_player_id BIGINT,
...
BEGIN
    ...
    ELSE
        ...
        FROM kopecht.teams t
        JOIN kopecht.team_season_rankings tsr ON t.id = tsr.team_id AND tsr.season_id = p_season_id
        WHERE (t.player1_id = p_player_id OR t.player2_id = p_player_id);
    END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
GRANT SELECT ON kopecht.team_status TO authenticated;
GRANT SELECT ON kopecht.team_bounty_history TO authenticated;
GRANT EXECUTE ON FUNCTION kopecht.update_team_status_after_match(BIGINT, BIGINT, BOOLEAN, BIGINT) TO authenticated;
GRANT EXECUTE ON FUNCTION kopecht.update_team_status_after_match(BIGINT, BIGINT, BOOLEAN, BIGINT) TO service_role;
GRANT EXECUTE ON FUNCTION kopecht.get_team_bounty_for_team(BIGINT) TO authenticated;
GRANT EXECUTE ON FUNCTION kopecht.get_player_team_stats(BIGINT, BIGINT) TO authenticated;
