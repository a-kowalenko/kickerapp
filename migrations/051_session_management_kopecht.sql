-- Migration 051: Session Management Functions (kopecht schema)
...
-- ============================================
CREATE OR REPLACE FUNCTION kopecht.get_user_sessions()
RETURNS TABLE (
    session_id uuid,
...
-- ============================================
CREATE OR REPLACE FUNCTION kopecht.terminate_session(target_session_id text)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = auth, kopecht
...
-- ============================================
CREATE OR REPLACE FUNCTION kopecht.terminate_other_sessions(current_session_id text)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = auth, kopecht
...
GRANT EXECUTE ON FUNCTION kopecht.get_user_sessions() TO authenticated;
GRANT EXECUTE ON FUNCTION kopecht.terminate_session(text) TO authenticated;
GRANT EXECUTE ON FUNCTION kopecht.terminate_other_sessions(text) TO authenticated;
