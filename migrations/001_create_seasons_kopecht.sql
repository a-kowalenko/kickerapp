...
CREATE TABLE IF NOT EXISTS kopecht.seasons (
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_seasons_kicker_id ON kopecht.seasons(kicker_id);
CREATE INDEX IF NOT EXISTS idx_seasons_is_active ON kopecht.seasons(is_active);

-- 2. Add current_season_id to kicker table
ALTER TABLE kopecht.kicker 
    ADD COLUMN IF NOT EXISTS current_season_id BIGINT REFERENCES kopecht.seasons(id);

-- 3. Create season_rankings table
CREATE TABLE IF NOT EXISTS kopecht.season_rankings (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_season_rankings_player_id ON kopecht.season_rankings(player_id);
CREATE INDEX IF NOT EXISTS idx_season_rankings_season_id ON kopecht.season_rankings(season_id);
...
BEGIN
    FOR k IN SELECT id FROM kopecht.kicker LOOP
        INSERT INTO kopecht.seasons (kicker_id, season_number, name, is_active, start_date)
        VALUES (k.id, 0, 'Season 0', TRUE, NOW())
        RETURNING id INTO new_season_id;

        UPDATE kopecht.kicker 
        ...
        INSERT INTO kopecht.season_rankings (player_id, season_id, wins, losses, mmr, wins2on2, losses2on2, mmr2on2)
...
CREATE OR REPLACE FUNCTION kopecht.get_next_season_number(p_kicker_id BIGINT)
RETURNS INT AS $$
DECLARE
    next_num INT;
BEGIN
...
CREATE OR REPLACE FUNCTION kopecht.get_player_matches_count(kicker_id BIGINT, p_season_id BIGINT DEFAULT NULL)
...
CREATE OR REPLACE FUNCTION kopecht.create_season_ranking_for_new_player()
RETURNS TRIGGER AS $$
DECLARE
...
CREATE TRIGGER after_player_insert_create_season_ranking
    AFTER INSERT ON kopecht.player
    FOR EACH ROW
    EXECUTE FUNCTION kopecht.create_season_ranking_for_new_player();

-- 8. get_season_rankings function
CREATE OR REPLACE FUNCTION kopecht.get_season_rankings(p_kicker_id BIGINT, p_season_id BIGINT)
RETURNS TABLE(
    id BIGINT,
    player_id BIGINT,
    name TEXT,
...
BEGIN
    ...
    JOIN kopecht.player p ON sr.player_id = p.id
...
-- 9. RLS
ALTER TABLE kopecht.seasons ENABLE ROW LEVEL SECURITY;
ALTER TABLE kopecht.season_rankings ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view seasons for their kickers" ON kopecht.seasons
    FOR SELECT USING (true);

CREATE POLICY "Users can view season_rankings" ON kopecht.season_rankings
    FOR SELECT USING (true);

CREATE POLICY "Admins can insert seasons" ON kopecht.seasons
    FOR INSERT WITH CHECK (true);

CREATE POLICY "Admins can update seasons" ON kopecht.seasons
    FOR UPDATE USING (true);

CREATE POLICY "System can manage season_rankings" ON kopecht.season_rankings
    FOR ALL USING (true);
