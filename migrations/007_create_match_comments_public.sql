-- Migration: Create match_comments, match_reactions, and comment_reactions tables
...
CREATE INDEX IF NOT EXISTS idx_match_comments_player_id ON match_comments(player_id);
CREATE INDEX IF NOT EXISTS idx_match_comments_kicker_id ON match_comments(kicker_id);
...
CREATE INDEX IF NOT EXISTS idx_match_reactions_player_id ON match_reactions(player_id);
...
CREATE INDEX IF NOT EXISTS idx_comment_reactions_player_id ON comment_reactions(player_id);
...
USING (
    EXISTS (
        ...
        WHERE player.kicker_id = match_comments.kicker_id
...
WITH CHECK (
    EXISTS (
        ...
        WHERE player.kicker_id = match_comments.kicker_id
        AND player.user_id = auth.uid()
        AND player.id = match_comments.player_id
    )
);
...
USING (
    EXISTS (
        SELECT 1 FROM player
        WHERE player.id = match_comments.player_id
        AND player.user_id = auth.uid()
    )
)
WITH CHECK (
...
USING (
    EXISTS (
        ...
        WHERE kicker.id = match_comments.kicker_id
...
USING (
    EXISTS (
        SELECT 1 FROM player
        WHERE player.kicker_id = match_reactions.kicker_id
        AND player.user_id = auth.uid()
    )
);
...
WITH CHECK (
    EXISTS (
        ...
        WHERE player.kicker_id = match_reactions.kicker_id
        AND player.user_id = auth.uid()
        AND player.id = match_reactions.player_id
    )
);
...
USING (
    EXISTS (
        SELECT 1 FROM player
        WHERE player.id = match_reactions.player_id
        AND player.user_id = auth.uid()
    )
);
...
USING (
    EXISTS (
        SELECT 1 FROM player
        WHERE player.kicker_id = comment_reactions.kicker_id
        AND player.user_id = auth.uid()
    )
);
...
WITH CHECK (
    EXISTS (
        ...
        WHERE player.kicker_id = comment_reactions.kicker_id
        AND player.user_id = auth.uid()
        AND player.id = comment_reactions.player_id
    )
);
...
USING (
    EXISTS (
        SELECT 1 FROM player
        WHERE player.id = comment_reactions.player_id
        AND player.user_id = auth.uid()
    )
);
