-- Migration: Update get_players_by_kicker to include season-specific MMR from season_rankings
...
DROP FUNCTION IF EXISTS get_players_by_kicker(INTEGER);

-- Create a custom type for the return value
DROP TYPE IF EXISTS player_with_season_mmr;
CREATE TYPE player_with_season_mmr AS (
    id BIGINT,
...
CREATE OR REPLACE FUNCTION get_players_by_kicker(kicker_id_param INTEGER, p_season_id BIGINT DEFAULT NULL)
RETURNS SETOF player_with_season_mmr AS $$
BEGIN
