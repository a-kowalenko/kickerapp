...
-- 2. Create indexes
CREATE INDEX IF NOT EXISTS team_season_rankings_team_id_idx ON public.team_season_rankings(team_id);
CREATE INDEX IF NOT EXISTS team_season_rankings_season_id_idx ON public.team_season_rankings(season_id);
...
ON public.team_season_rankings
AS PERMISSIVE
FOR SELECT
TO public
USING (
...
-- 5. Grant permissions
GRANT SELECT, INSERT, UPDATE, DELETE ON public.team_season_rankings TO authenticated;
GRANT USAGE ON SEQUENCE public.team_season_rankings_id_seq TO authenticated;

-- 6. Function to get or create team season ranking
CREATE OR REPLACE FUNCTION public.get_or_create_team_season_ranking(
    p_team_id BIGINT,
    p_season_id BIGINT
)
RETURNS public.team_season_rankings
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
    v_ranking public.team_season_rankings;
BEGIN
    -- Try to get existing ranking
    SELECT * INTO v_ranking
    FROM public.team_season_rankings
    WHERE team_id = p_team_id AND season_id = p_season_id;
    
    -- If not found, create it
    IF v_ranking IS NULL THEN
        INSERT INTO public.team_season_rankings (team_id, season_id, wins, losses, mmr)
...
CREATE OR REPLACE FUNCTION public.update_team_season_ranking(
    p_team_id BIGINT,
    p_season_id BIGINT,
...
CREATE OR REPLACE FUNCTION public.get_team_season_rankings(
    p_kicker_id BIGINT,
    p_season_id BIGINT
)
RETURNS TABLE (
...
BEGIN
    ...
    LEFT JOIN public.team_season_rankings tsr ON t.id = tsr.team_id AND tsr.season_id = p_season_id
...
GRANT EXECUTE ON FUNCTION public.get_or_create_team_season_ranking(BIGINT, BIGINT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.update_team_season_ranking(BIGINT, BIGINT, INTEGER, BOOLEAN) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_team_season_rankings(BIGINT, BIGINT) TO authenticated;
