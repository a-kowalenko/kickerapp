-- Migration: Create Seasons Feature
-- Date: 2024-12-09
-- Description: Adds seasons support for kicker tracking
...
ALTER TABLE kicker ADD COLUMN IF NOT EXISTS current_season_id BIGINT REFERENCES seasons(id);
...
BEGIN
    FOR k IN SELECT id FROM kicker LOOP
        ...
        UPDATE kicker SET current_season_id = new_season_id WHERE id = k.id;
        
        -- Assign all existing matches to Season 0
        UPDATE matches SET season_id = new_season_id WHERE kicker_id = k.id AND season_id IS NULL;
        
        -- Copy current player stats to season_rankings
        ...
        FROM player WHERE kicker_id = k.id;
    END LOOP;
END $$;

-- 5. Create function to auto-increment season_number per kicker
...
-- 6. Update get_player_matches_count to support optional season_id filter
...
-- 7. Create trigger function to auto-create season_rankings entry for new players
CREATE OR REPLACE FUNCTION create_season_ranking_for_new_player()
RETURNS TRIGGER AS $$
DECLARE
    current_season BIGINT;
BEGIN
    -- Get the current active season for the kicker
    SELECT current_season_id INTO current_season
    FROM kicker
    WHERE id = NEW.kicker_id;
    
    -- If there's an active season, create a season_rankings entry
...
-- Create trigger on player table
...
-- 8. Create function to get season rankings (for use in rankings queries)
...
-- 9. Enable RLS for new tables (if RLS is used)
ALTER TABLE seasons ENABLE ROW LEVEL SECURITY;
ALTER TABLE season_rankings ENABLE ROW LEVEL SECURITY;

-- Create policies (adjust based on your existing RLS setup)
