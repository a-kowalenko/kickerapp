-- Rollback: Remove team invite notifications from mention_notifications system (kopecht schema)
...
DROP FUNCTION IF EXISTS kopecht.trigger_mark_team_invite_notification_read();
DROP FUNCTION IF EXISTS kopecht.trigger_create_team_invite_notification();

-- 3. Delete any existing team_invite notifications
DELETE FROM kopecht.mention_notifications WHERE type = 'team_invite';

-- 4. Drop team_invitation_id index and column
DROP INDEX IF EXISTS kopecht.idx_mention_notifications_team_invitation_id;
ALTER TABLE kopecht.mention_notifications DROP COLUMN IF EXISTS team_invitation_id;
