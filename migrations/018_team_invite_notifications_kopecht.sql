-- Migration: Add team invite notifications to mention_notifications system (kopecht schema)
...
ALTER TABLE kopecht.mention_notifications 
ADD COLUMN IF NOT EXISTS team_invitation_id BIGINT REFERENCES kopecht.team_invitations(id) ON DELETE CASCADE;

-- Create index for team_invitation_id lookups
CREATE INDEX IF NOT EXISTS idx_mention_notifications_team_invitation_id 
ON kopecht.mention_notifications(team_invitation_id) 
...
CREATE OR REPLACE FUNCTION kopecht.trigger_mark_team_invite_notification_read()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
DROP TRIGGER IF EXISTS trigger_team_invite_response ON kopecht.team_invitations;
CREATE TRIGGER trigger_team_invite_response
    AFTER UPDATE ON kopecht.team_invitations
    FOR EACH ROW
    EXECUTE FUNCTION kopecht.trigger_mark_team_invite_notification_read();
