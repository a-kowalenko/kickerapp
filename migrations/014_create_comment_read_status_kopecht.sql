...
CREATE TABLE IF NOT EXISTS kopecht.comment_read_status (
...
CREATE INDEX IF NOT EXISTS idx_comment_read_status_user_id ON kopecht.comment_read_status(user_id);
CREATE INDEX IF NOT EXISTS idx_comment_read_status_kicker_id ON kopecht.comment_read_status(kicker_id);
CREATE INDEX IF NOT EXISTS idx_comment_read_status_last_read_at ON kopecht.comment_read_status(last_read_at);

-- 2. Enable Row Level Security
ALTER TABLE kopecht.comment_read_status ENABLE ROW LEVEL SECURITY;

-- 3. RLS Policies for comment_read_status

-- Users can view their own read status
CREATE POLICY "Users can view own comment read status"
ON kopecht.comment_read_status FOR SELECT
USING (auth.uid() = user_id);

-- Users can insert their own read status
CREATE POLICY "Users can insert own comment read status"
ON kopecht.comment_read_status FOR INSERT
WITH CHECK (auth.uid() = user_id);

-- Users can update their own read status
CREATE POLICY "Users can update own comment read status"
ON kopecht.comment_read_status FOR UPDATE
USING (auth.uid() = user_id)
WITH CHECK (auth.uid() = user_id);

-- Users can delete their own read status
CREATE POLICY "Users can delete own comment read status"
ON kopecht.comment_read_status FOR DELETE
USING (auth.uid() = user_id);

-- 4. Create function to update last_read_at and updated_at
CREATE OR REPLACE FUNCTION kopecht.update_comment_read_status(p_kicker_id BIGINT)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    INSERT INTO kopecht.comment_read_status (user_id, kicker_id, last_read_at, updated_at)
...
CREATE OR REPLACE FUNCTION kopecht.get_unread_comment_count_per_kicker()
...
BEGIN
    ...
    LEFT JOIN kopecht.comment_read_status crs ON crs.kicker_id = p.kicker_id AND crs.user_id = auth.uid()
...
CREATE OR REPLACE FUNCTION kopecht.get_unread_comment_count(p_kicker_id BIGINT)
RETURNS BIGINT
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    FROM kopecht.comment_read_status
    WHERE user_id = auth.uid() AND kicker_id = p_kicker_id;
...
ALTER PUBLICATION supabase_realtime ADD TABLE kopecht.comment_read_status;

-- 8. Grant execute permissions on functions
GRANT EXECUTE ON FUNCTION kopecht.update_comment_read_status(BIGINT) TO authenticated;
GRANT EXECUTE ON FUNCTION kopecht.get_unread_comment_count_per_kicker() TO authenticated;
GRANT EXECUTE ON FUNCTION kopecht.get_unread_comment_count(BIGINT) TO authenticated;
