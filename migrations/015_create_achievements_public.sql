...
CREATE TABLE IF NOT EXISTS public.achievement_definitions (
    ...
    category_id BIGINT NOT NULL REFERENCES public.achievement_categories(id) ON DELETE CASCADE,
    ...
    parent_id BIGINT REFERENCES public.achievement_definitions(id) ON DELETE SET NULL,
    sort_order INT NOT NULL DEFAULT 0,
    kicker_id BIGINT NOT NULL REFERENCES public.kicker(id) ON DELETE CASCADE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE INDEX IF NOT EXISTS idx_achievement_categories_kicker ON public.achievement_categories(kicker_id);
CREATE INDEX IF NOT EXISTS idx_achievement_definitions_kicker ON public.achievement_definitions(kicker_id);
CREATE INDEX IF NOT EXISTS idx_achievement_definitions_trigger ON public.achievement_definitions(trigger_event);
CREATE INDEX IF NOT EXISTS idx_achievement_definitions_category ON public.achievement_definitions(category_id);
CREATE INDEX IF NOT EXISTS idx_achievement_definitions_parent ON public.achievement_definitions(parent_id);
CREATE INDEX IF NOT EXISTS idx_achievement_definitions_season ON public.achievement_definitions(season_id);
CREATE INDEX IF NOT EXISTS idx_player_achievement_progress_player ON public.player_achievement_progress(player_id);
CREATE INDEX IF NOT EXISTS idx_player_achievement_progress_achievement ON public.player_achievement_progress(achievement_id);
...
CREATE INDEX IF NOT EXISTS idx_player_achievements_season ON public.player_achievements(season_id);
CREATE INDEX IF NOT EXISTS idx_player_achievements_unlocked ON public.player_achievements(unlocked_at DESC);
...
CREATE POLICY "Admins can manage achievement categories"
    ON public.achievement_categories FOR ALL
    USING (
        EXISTS (
...
CREATE POLICY "Service role can manage all progress"
    ON public.player_achievement_progress FOR ALL
...
CREATE OR REPLACE FUNCTION public.update_achievement_definition_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
CREATE TRIGGER update_achievement_definitions_updated_at
    ...
    FOR EACH ROW
    EXECUTE FUNCTION public.update_achievement_definition_updated_at();
...
CREATE TRIGGER update_player_achievement_progress_updated_at
    BEFORE UPDATE ON public.player_achievement_progress
    FOR EACH ROW
