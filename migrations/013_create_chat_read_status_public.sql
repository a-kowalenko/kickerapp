...
CREATE INDEX IF NOT EXISTS idx_chat_read_status_kicker_id ON chat_read_status(kicker_id);
CREATE INDEX IF NOT EXISTS idx_chat_read_status_last_read_at ON chat_read_status(last_read_at);
...
ON chat_read_status FOR DELETE
USING (auth.uid() = user_id);

-- 4. Create function to update last_read_at and updated_at
CREATE OR REPLACE FUNCTION update_chat_read_status(p_kicker_id BIGINT)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE OR REPLACE FUNCTION get_unread_count_per_kicker()
...
CREATE OR REPLACE FUNCTION get_unread_count_for_user(p_user_id UUID)
RETURNS BIGINT
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
GRANT EXECUTE ON FUNCTION update_chat_read_status(BIGINT) TO authenticated;
GRANT EXECUTE ON FUNCTION get_unread_count_per_kicker() TO authenticated;
GRANT EXECUTE ON FUNCTION get_total_unread_count() TO authenticated;
GRANT EXECUTE ON FUNCTION get_unread_count_for_user(UUID) TO service_role;
