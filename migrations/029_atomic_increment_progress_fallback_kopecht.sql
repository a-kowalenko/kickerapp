    ...
    IF p_season_id IS NULL THEN
        ...
        DO UPDATE SET 
            current_progress = LEAST(
                kopecht.player_achievement_progress.current_progress + p_increment, 
                p_max_progress
            ),
            updated_at = NOW()
...
COMMENT ON FUNCTION kopecht.atomic_increment_progress IS 'Atomic fallback for incrementing achievement progress. Uses advisory lock to prevent race conditions under high concurrency.';
