...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_seasons_kicker_id ON public.seasons(kicker_id);
...
ALTER TABLE public.kicker 
    ADD COLUMN IF NOT EXISTS current_season_id BIGINT REFERENCES public.seasons(id);
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_season_rankings_player_id ON public.season_rankings(player_id);
CREATE INDEX IF NOT EXISTS idx_season_rankings_season_id ON public.season_rankings(season_id);
...
BEGIN
    FOR k IN SELECT id FROM public.kicker LOOP
        INSERT INTO public.seasons (kicker_id, season_number, name, is_active, start_date)
        VALUES (k.id, 0, 'Season 0', TRUE, NOW())
        RETURNING id INTO new_season_id;

        UPDATE public.kicker 
...
CREATE OR REPLACE FUNCTION public.get_next_season_number(p_kicker_id BIGINT)
RETURNS INT AS $$
DECLARE
    next_num INT;
BEGIN
...
CREATE OR REPLACE FUNCTION public.get_player_matches_count(kicker_id BIGINT, p_season_id BIGINT DEFAULT NULL)
...
CREATE OR REPLACE FUNCTION public.create_season_ranking_for_new_player()
RETURNS TRIGGER AS $$
DECLARE
...
CREATE TRIGGER after_player_insert_create_season_ranking
    ...
    FOR EACH ROW
    EXECUTE FUNCTION public.create_season_ranking_for_new_player();

-- 8. get_season_rankings function
CREATE OR REPLACE FUNCTION public.get_season_rankings(p_kicker_id BIGINT, p_season_id BIGINT)
RETURNS TABLE(
    id BIGINT,
    player_id BIGINT,
    name TEXT,
...
CREATE POLICY "Users can view seasons for their kickers" ON public.seasons
    FOR SELECT USING (true);

CREATE POLICY "Users can view season_rankings" ON public.season_rankings
    FOR SELECT USING (true);
...
CREATE POLICY "System can manage season_rankings" ON public.season_rankings
    FOR ALL USING (true);
