-- Migration: Create team_history table for team MMR tracking (public schema)
...
CREATE INDEX IF NOT EXISTS idx_team_history_team_id ON public.team_history(team_id);
CREATE INDEX IF NOT EXISTS idx_team_history_match_id ON public.team_history(match_id);
CREATE INDEX IF NOT EXISTS idx_team_history_created_at ON public.team_history(created_at DESC);
...
CREATE TRIGGER trigger_record_team_history
    AFTER UPDATE OF end_time ON public.matches
    FOR EACH ROW
    WHEN (OLD.end_time IS NULL AND NEW.end_time IS NOT NULL)
    EXECUTE FUNCTION public.record_team_history();
...
GRANT SELECT ON public.team_history TO anon;

-- 8. Create RPC function to get team MMR history
CREATE OR REPLACE FUNCTION public.get_team_mmr_history(
    p_team_id BIGINT,
...
GRANT EXECUTE ON FUNCTION public.get_team_mmr_history(BIGINT, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_team_mmr_history(BIGINT, INTEGER) TO anon;
