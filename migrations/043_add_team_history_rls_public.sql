-- Migration: Add RLS policies for public.team_history table
...
DROP POLICY IF EXISTS "access_control_on_team_history" ON public.team_history;
DROP POLICY IF EXISTS "insert_control_on_team_history" ON public.team_history;
DROP POLICY IF EXISTS "update_control_on_team_history" ON public.team_history;
DROP POLICY IF EXISTS "delete_control_on_team_history" ON public.team_history;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.team_history TO authenticated;
GRANT USAGE ON SEQUENCE public.team_history_id_seq TO authenticated;
