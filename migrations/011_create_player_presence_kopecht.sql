...
CREATE TABLE IF NOT EXISTS kopecht.player_presence (
    player_id BIGINT PRIMARY KEY REFERENCES kopecht.player(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_player_presence_kicker_id ON kopecht.player_presence(kicker_id);

-- Create index for efficient queries by last_seen (for inactive player queries)
CREATE INDEX IF NOT EXISTS idx_player_presence_last_seen ON kopecht.player_presence(last_seen);

-- Enable RLS
ALTER TABLE kopecht.player_presence ENABLE ROW LEVEL SECURITY;

-- Policy: Allow authenticated users to read all presence data
CREATE POLICY "Allow authenticated users to read presence"
    ON kopecht.player_presence
    FOR SELECT
    TO authenticated
    USING (true);
...
CREATE OR REPLACE FUNCTION kopecht.upsert_player_presence(
    p_player_id BIGINT,
    p_kicker_id BIGINT
)
RETURNS void
...
AS $$
BEGIN
    INSERT INTO kopecht.player_presence (player_id, kicker_id, last_seen)
...
CREATE OR REPLACE FUNCTION kopecht.get_players_activity(p_kicker_id BIGINT)
RETURNS TABLE (
    player_id BIGINT,
    player_name TEXT,
    player_avatar TEXT,
...
BEGIN
    ...
    LEFT JOIN kopecht.player_presence pp ON p.id = pp.player_id
...
GRANT EXECUTE ON FUNCTION kopecht.upsert_player_presence(BIGINT, BIGINT) TO authenticated;
GRANT EXECUTE ON FUNCTION kopecht.get_players_activity(BIGINT) TO authenticated;
