...
DROP FUNCTION IF EXISTS public.get_achievement_feed_stats(BIGINT, BIGINT);
DROP FUNCTION IF EXISTS public.get_achievement_leaderboard(BIGINT, BIGINT, INT);

-- Function to get achievement feed statistics
-- Returns counts for today, this week, this month, and total
CREATE OR REPLACE FUNCTION public.get_achievement_feed_stats(
    p_kicker_id BIGINT,
...
BEGIN
    ...
    INNER JOIN public.player p ON p.id = pa.player_id
...
BEGIN
    ...
    INNER JOIN public.player_achievements pa ON pa.player_id = p.id
    INNER JOIN public.achievement_definitions ad ON ad.id = pa.achievement_id
    WHERE p.kicker_id = p_kicker_id
      AND (p_season_id IS NULL OR pa.season_id = p_season_id)
...
GRANT EXECUTE ON FUNCTION public.get_achievement_feed_stats(BIGINT, BIGINT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_achievement_leaderboard(BIGINT, BIGINT, INT) TO authenticated;

-- Add helpful comments
COMMENT ON FUNCTION public.get_achievement_feed_stats IS 'Get achievement unlock statistics for a kicker (counts by time period)';
COMMENT ON FUNCTION public.get_achievement_leaderboard IS 'Get top players by achievement points for a kicker';
