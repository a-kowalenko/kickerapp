-- Rollback: 001_create_seasons_sql
...
DROP POLICY IF EXISTS "System can manage season_rankings" ON season_rankings;
DROP POLICY IF EXISTS "Admins can update seasons" ON seasons;
DROP POLICY IF EXISTS "Admins can insert seasons" ON seasons;
DROP POLICY IF EXISTS "Users can view season_rankings" ON season_rankings;
DROP POLICY IF EXISTS "Users can view seasons for their kickers" ON seasons;

-- 2. Drop get_season_rankings function
DROP FUNCTION IF EXISTS get_season_rankings(BIGINT, BIGINT);
...
DROP FUNCTION IF EXISTS create_season_ranking_for_new_player();

-- 4. Restore original get_player_matches_count function (without season support)
CREATE OR REPLACE FUNCTION get_player_matches_count(kicker_id BIGINT)
...
DROP FUNCTION IF EXISTS get_next_season_number(BIGINT);
...
ALTER TABLE kicker DROP COLUMN IF EXISTS current_season_id;
