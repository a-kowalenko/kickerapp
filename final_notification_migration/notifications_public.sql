...
GRANT EXECUTE ON FUNCTION delete_push_subscription TO authenticated;
...
CREATE OR REPLACE FUNCTION trigger_create_chat_all_notifications()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
DROP TRIGGER IF EXISTS trigger_chat_all ON chat_messages;
CREATE TRIGGER trigger_chat_all
    AFTER INSERT ON chat_messages
    FOR EACH ROW
    EXECUTE FUNCTION trigger_create_chat_all_notifications();
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION get_mention_notifications(INT, INT) TO authenticated;
...
CREATE OR REPLACE FUNCTION set_global_notifications_enabled(
    p_enabled BOOLEAN
)
RETURNS BOOLEAN
LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION set_device_notifications_enabled(
    p_subscription_id BIGINT,
    p_enabled BOOLEAN
)
RETURNS BOOLEAN
...
GRANT EXECUTE ON FUNCTION set_global_notifications_enabled(BOOLEAN) TO authenticated;
GRANT EXECUTE ON FUNCTION set_device_notifications_enabled(BIGINT, BOOLEAN) TO authenticated;
...
DROP TRIGGER IF EXISTS trigger_send_push_notification ON mention_notifications;
CREATE TRIGGER trigger_send_push_notification
    AFTER INSERT ON mention_notifications
    FOR EACH ROW
...
CREATE TRIGGER trigger_chat_mentions
    AFTER INSERT ON chat_messages
    FOR EACH ROW
    EXECUTE FUNCTION trigger_create_chat_mention_notifications();
...
CREATE TRIGGER trigger_comment_mentions
    AFTER INSERT ON match_comments
    FOR EACH ROW
    EXECUTE FUNCTION trigger_create_comment_mention_notifications();
...
REFERENCES team_invitations(id) 
ON DELETE CASCADE;
