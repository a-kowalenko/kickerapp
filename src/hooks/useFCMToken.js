...
export function useFCMToken(userId) {
    ...
    const enableNotifications = useCallback(async () => {
        ...
        try {
            ...
            // Save token to database with complete device info
            saveToken({
                token,
    ...
    }, [userId, notificationStatus.supported, saveToken]);
    ...
    useEffect(() => {
        ...
        const checkToken = async () => {
            ...
            if (!existingToken) {
                // Token is new or has changed, save it with complete device info
                saveToken({
                    token,
                    deviceInfo: getDeviceInfo(),
                });
