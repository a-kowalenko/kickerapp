...
import {
    useAdminPlayerAchievements,
    useDeleteAdminPlayerAchievement,
} from "./usePlayerAchievementsAdmin";
import PlayerUnlockedModal from "./PlayerUnlockedModal";
...
const TooltipPoints = styled.div`
    margin-top: 0.6rem;
    font-size: 1.1rem;
...
const MatchTooltipTeams = styled.div`
    display: flex;
    align-items: center;
...
const MatchTooltipTeam = styled.span`
    font-size: 1.2rem;
    color: var(--secondary-text-color);
`;

const MatchTooltipScore = styled.span`
    font-size: 1.4rem;
    font-weight: 700;
    color: var(--primary-text-color);
`;
...
const MatchHoverTrigger = styled.span`
    cursor: help;
`;
        ...
        >
            <TooltipContent>
                ...
                {achievement.points > 0 && (
                    <TooltipPoints>{achievement.points} points</TooltipPoints>
                )}
            </TooltipContent>
        </TooltipContainer>,
        document.body
...
/* ----------------------------------------
   Match Tooltip Component
----------------------------------------- */
function MatchTooltip({ match, isVisible, position }) {
        ...
        >
            <TooltipContent>
                ...
                <TooltipHeader>
                    ...
                    <TooltipTitle>Match #{match.nr}</TooltipTitle>
                </TooltipHeader>
                <MatchTooltipTeams>
                    <MatchTooltipTeam>{team1}</MatchTooltipTeam>
                    <MatchTooltipScore>
                        {match.scoreTeam1} : {match.scoreTeam2}
                    </MatchTooltipScore>
                    <MatchTooltipTeam>{team2}</MatchTooltipTeam>
                </MatchTooltipTeams>
            </TooltipContent>
        </TooltipContainer>,
        document.body
    ...
    return (
        <>
            <MatchHoverTrigger
                ...
                onMouseLeave={handleMouseLeave}
            >
                <Badge $variant="info">#{match.nr}</Badge>
            </MatchHoverTrigger>
            <MatchTooltip
                match={match}
                isVisible={isHovered}
...
function PlayerUnlockedTab() {
    ...
    const { playerAchievements, isLoading: isLoadingAchievements } =
        useAdminPlayerAchievements({
    ...
    const handleDelete = (id) => {
        if (
            window.confirm(
                "Are you sure you want to delete this player achievement?"
            )
        ) {
            ...
            <Section>
                <SectionHeader>
                    <SectionTitle>
                        Player Achievements{" "}
                        <ResultCount>
                            ({playerAchievements?.length || 0} entries)
                        </ResultCount>
                    </SectionTitle>
                ...
                {playerAchievements && playerAchievements.length > 0 ? (
                            ...
                            <tbody>
                                {playerAchievements.map((item) => (
                                    <tr key={item.id}>
                                        ...
                                        <Td>{formatDate(item.unlocked_at)}</Td>
                                        <Td>
                                            <Actions>
                                                <IconButton
                                                    onClick={() =>
                        ...
                        <MobileList>
                            {playerAchievements.map((item) => (
                                <MobileCard key={item.id}>
                                    ...
                                    <MobileCardDetail>
                                        <span>Times:</span>
                                        <span>{item.times_completed}x</span>
                                    </MobileCardDetail>
                                    <MobileCardDetail>
                                        <span>Unlocked:</span>
                                        <span>
                    ...
                    <NoData>
                        No player achievements found. Try adjusting your filters
                        or add a new entry.
                    </NoData>
                )}
            </Section>
...
export default PlayerUnlockedTab;
